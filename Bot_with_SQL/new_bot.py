...
# Database client class for MySQL
class AdsMySQLClient:
    def __init__(self, host: str, user: str, password: str, database: str):
        ...
        self.pool = mysql.connector.pooling.MySQLConnectionPool(
            pool_name="mypool",
            pool_size=3,  # Set to 3 to match the max_user_connections limit
    ...
    def add_category(self, category: str):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("INSERT IGNORE INTO categories (category) VALUES (%s)", (category,))
            conn.commit()
        finally:
            cursor.close()
            conn.close()
            logging.info("Connection closed after adding category")
    ...
    def get_categories(self) -> List[str]:
        ...
        try:
            ...
            cursor.close()
            conn.close()
            logging.info("Connection closed after getting categories")
    ...
    def add_advertising(self, user_id: int, photo_url: str, category: str, description: str):
        ...
        try:
            cursor.execute(
                "INSERT INTO ads (user_id, photo_url, category, description) VALUES (%s, %s, %s, %s)",
                (user_id, photo_url, category, description)
            )
            conn.commit()
        finally:
            cursor.close()
            conn.close()
            logging.info("Connection closed after adding advertising")

    def delete_advertising(self, user_id: int, ad_id: int):
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute("DELETE FROM ads WHERE user_id = %s AND id = %s", (user_id, ad_id))
            conn.commit()
        finally:
            cursor.close()
            conn.close()
            logging.info("Connection closed after deleting advertising")

    def get_ads_by_user_id(self, user_id: int):
        conn = self.get_connection()
        cursor = conn.cursor(dictionary=True)
        try:
            cursor.execute("SELECT id, photo_url, category, description FROM ads WHERE user_id = %s", (user_id,))
            results = cursor.fetchall()
            return results
        finally:
            cursor.close()
            conn.close()
            logging.info("Connection closed after getting ads by user ID")

    def get_ads_by_category(self, category: str):
        conn = self.get_connection()
        cursor = conn.cursor(dictionary=True)
        try:
            cursor.execute("SELECT id, photo_url, category, description FROM ads WHERE category LIKE %s", ('%' + category + '%',))
            results = cursor.fetchall()
            return results
        finally:
            cursor.close()
            conn.close()
            logging.info("Connection closed after getting ads by category")

    def update_description(self, ad_id: int, edited_description: str):
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute("UPDATE ads SET description = %s WHERE id = %s", (edited_description, ad_id))
            conn.commit()
        finally:
            cursor.close()
            conn.close()
            logging.info("Connection closed after updating description")

    def close(self):
        pass  # No need for a close method with manual connection management


# Initialize the database client
db_client = AdsMySQLClient(
    host="Shayan2101.mysql.pythonanywhere-services.com",
    user="Shayan2101",
    password="13$h@Y@n81",
    database="Shayan2101$ads_database"
)
